    ...
    // Server socket class
    {
        ...
        private void SendCallback(IAsyncResult ar)
        {
            try
            {
                ...
                //CLogger.Add(LOG.COMM, "Sent {0} bytes to client.", bytesSent);                
        ...
        public bool Connect(string sAddr, int nPort)
        {
            ...
            if (socketClient == null)
            {
                ...
                CLOG.COMM($"Create new stocket instance {sAddr}:{nPort}");                
            }

            try
            {
                CLOG.COMM($"!! BeginConnect to {sAddr}:{nPort}");
