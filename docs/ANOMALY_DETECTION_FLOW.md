...
## 7. YOLOv8 / YOLO11 / PatchCore 동일 데이터 비교

1. 같은 Recipe에서 Normal/Abnormal 판정과 test split을 먼저 고정합니다.
2. 모델 프로필에서 YOLOv8, YOLO11 또는 PatchCore를 선택하고 Model Center의
   `평가 실행`을 누릅니다.
3. 다른 모델로 전환한 뒤 같은 데이터 상태에서 `평가 실행`을 반복합니다.
4. `모델 성능 비교`를 열면 현재 프로젝트에서 데이터 fingerprint가 같은
   이상탐지 실행들이 함께 선택됩니다.
5. balanced accuracy, 정상 오검출, 이상 미검출, weights/worker SHA-256,
   장비/실행 조건과 이미지당 시간을 확인합니다.
6. PatchCore는 위치 후보 수와 히트맵 근거도 표시하지만, 위치 정답이 없으면
   위치 정확도는 `평가되지 않음`입니다.

YOLO confidence와 PatchCore anomaly score는 의미가 다르므로 숫자 크기를 직접
비교하지 않습니다. 비교 화면은 읽기 전용이며 모델을 자동 채택하거나 라벨을
저장하지 않습니다. 실제 모델 선정은 출처가 확인된 독립 현장 이미지와 합의된
오검출/미검출 기준으로 같은 절차를 다시 실행한 뒤 결정합니다.

## 8. 평가 오류 이미지 확인

1. `모델 성능 비교`의 `요약`에서 모델별 정답, 정상 오검출, 이상 미검출 수를
   확인합니다.
2. `클래스/오류` 탭을 열어 `이미지별 판정 결과`를 확인합니다.
3. 표는 오검출, 미검출, 임계값 미달을 정답보다 먼저 보여줍니다.
4. 행을 선택하면 저장된 원본 이미지와 `정상 → 이상` 같은 판정 흐름, 신뢰도
   또는 PatchCore 이상 점수와 임계값을 확인할 수 있습니다.
5. PatchCore는 위치 후보 수와 히트맵 기록 유무도 보여줍니다. 히트맵이 있다는
   사실만으로 위치 정확도가 검증된 것은 아닙니다.

표시는 최대 500건으로 제한되지만 보고서의 전체 집계 수치는 유지됩니다. 이
화면은 추론을 다시 실행하거나 라벨·검토 상태·모델 설정을 변경하지 않습니다.
