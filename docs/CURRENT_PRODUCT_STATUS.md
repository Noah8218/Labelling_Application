...
| OpenVisionLab Vision SDK consumer integration | Complete for SDK `3.0.0` | `OpenVisionLab.Core` and `OpenVisionLab.Vision2D` replace the active `Lib.Common`/`Lib.OpenCV` contract; clean-source package `0.1.4` records commit `c159014`, `source.dirty=false`, and 505 verified payloads; the app retains its pinned OpenCvSharp runtime and UI-only adapters |
