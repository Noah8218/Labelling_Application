...
2. 결함 클래스를 선택합니다.
3. 캔버스 상단 `라벨링 옵션`에서 `자동 윤곽`을 한 번 켭니다. 이
   선택은 현재 Recipe에 저장됩니다.
4. 사각형 도구로 결함을 여유 있게 감쌉니다. 사각형을 완성하면
   MobileSAM 후보가 자동 생성됩니다.
5. 파란 후보 외곽선을 확인합니다. 맞으면 `확정`, 틀리면 `스킵`합니다.
6. 후보가 부족할 때만 `보정 옵션`을 엽니다. 빠진 영역에는
   `+ 포함점`, 잘못 포함된 영역에는 `− 제외점`을 선택하고 한 번에
   한 점씩 캔버스를 클릭합니다.
7. `점 취소` 또는 `점 지우기`로 프롬프트를 고치고, `경계`에서
   `빠름(48)`, `균형(96)`, `상세(256)` 중 하나를 고른 뒤
   `후보 다시 생성`을 누릅니다.
8. 재생성 뒤 `이전 후보 보기`와 `현재 후보 보기`를 반복해 비교합니다.
   표시 중인 후보가 확정 대상입니다. 재실행과 후보 전환은 대기 후보를
   추가하거나 파일을 저장하지 않습니다.
9. 맞으면 `확정`, 틀리면 `스킵`합니다. 후보 생성 중에는 `생성 취소`로
   worker를 중단할 수 있으며 현재 프롬프트는 유지됩니다.
10. 자동 윤곽이 켜져 있으면 확정 또는 스킵 뒤 다음 사각형 입력으로
    바로 돌아갑니다. 수동 세션에서는 `다음 객체`로 돌아갑니다.
11. `확정`은 현재 표시 중인 후보를 기존 정답 저장 흐름으로 저장합니다.
    이후 수동으로 다시 편집했다면 `라벨 저장`을 눌러 최신 상태를
    기록합니다.
...
Recipe를 다시 열면 `자동 윤곽` 선택은 보이고 수정 가능한 상태로
복원됩니다. 복원 자체는 도구 선택, 사각형 생성, 추론, 후보 확정 또는
정답 저장을 실행하지 않습니다. 일반 박스 라벨을 만들려면 자동 윤곽을
끕니다.
...
- 완료: 실제 이미지 박스 입력 → 포함/제외점 보정 → rerun-replace →
  이전/현재 후보 비교·복원 → 사람 검토·확정/스킵 → 다음 객체 →
  기존 canonical 세그먼트 JSON·마스크 PNG 저장.
- 완료: Recipe-scoped 자동 윤곽 복원과 사각형 완성 직후 후보 생성.
  설정 복원만으로는 추론·확정·저장을 실행하지 않음.
