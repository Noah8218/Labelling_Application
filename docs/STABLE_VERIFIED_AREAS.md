...
- Keep the shared status/report/split/class result types in `YoloDatasetHealthContracts.cs` and the purpose-aware aggregation policy in `YoloDatasetHealthService.cs`.
...
- Keep image label status in `YoloImageLabelStatusContracts.cs` and the quality-report export result in `YoloImageQualityReviewReportExportContracts.cs`; keep artifact lookup/counting and Markdown aggregation/writing in their matching services.
...
- `Yolo/YoloImageLabelStatusContracts.cs`
...
- `Yolo/YoloDatasetReadinessContracts.cs`
...
- `Yolo/YoloImageReviewStatusContracts.cs`
...
2026-07-26 COCO/Label Studio export ownership contract: keep the public result and serialized document/task DTOs in `CocoDetectionExportContracts.cs`, `CocoSegmentationExportContracts.cs`, `LabelStudioDetectionExportContracts.cs`, and `LabelStudioSegmentationExportContracts.cs`. Keep dataset discovery, coordinate conversion, invalid-record filtering, relative-path construction, and JSON writing in the matching `*ExportService.cs`. Do not merge detection and segmentation DTOs merely because their shapes are similar; their external format semantics and focused gates are independent. Covered by the isolated build, all four format export gates, `--export-capability-inventory`, and `--priority-workflow-docs`.

2026-07-27 COCO/Label Studio/Pascal VOC interchange result ownership contract: keep COCO detection/segmentation import results together in `CocoImportContracts.cs`, Label Studio detection/segmentation import results in `LabelStudioImportContracts.cs`, and Pascal VOC detection export/import results in `PascalVocDetectionInterchangeContracts.cs`. Keep source parsing, class-catalog updates, coordinate conversion, image copy, local artifact writes, and `data.yaml` persistence in the matching services. Grouping is by stable external format, not DTO shape. Covered by the isolated build, the six matching import/export gates, `--export-capability-inventory`, and `--priority-workflow-docs`.

2026-07-27 CVAT interchange result ownership contract: keep detection/segmentation archive export results together in `CvatArchiveExportContracts.cs` and detection/segmentation import results in `CvatImportContracts.cs`. Keep XML/ZIP construction and parsing, image copying, class-catalog updates, coordinate conversion, local artifact writes, and `data.yaml` persistence in the matching CVAT services. The private archive image/box/polygon implementation types remain with their exporter because they are not public interchange contracts. Covered by the isolated build, all four CVAT import/export gates, `--export-capability-inventory`, and `--priority-workflow-docs`.
...
2026-07-27 export capability inventory ownership contract: keep the public format key, display name, direction, dataset purpose, implementation/recommendation flags, requirement summary, and verification switch item shape in `DatasetExportCapabilityContracts.cs`. Keep the implemented/planned entries, their order and values, filtering, and single recommended-next selection in `DatasetExportCapabilityService.cs`. This split must not change current availability or recommendation policy consumed by Model Adapter catalog. Covered by the isolated build, `--export-capability-inventory`, `--model-adapter-catalog`, `--wpf-yolo-model-settings-panel`, and `--priority-workflow-docs`.
...
2026-07-27 dataset quality audit ownership contract: keep the public report/split summary types in `YoloDatasetQualityAuditContracts.cs` and the Markdown export result in `YoloDatasetQualityAuditExportContracts.cs`. Keep image/label discovery, missing/empty/invalid classification, class counting, Markdown construction, and file writing in their matching services. This split must not change quality semantics, dashboard wording, or the rule that export writes only the requested report. Covered by the isolated build, `--dataset-quality-audit`, `--dataset-quality-audit-export`, `--dataset-health`, `--wpf-training-dashboard-quality`, and `--priority-workflow-docs`.
...
2026-07-27 anomaly classification evaluation ownership contract: keep held-out sample, evaluation threshold options, and report types in `AnomalyClassificationEvaluationContracts.cs`; keep summary JSON parsing, low-confidence handling, count/accuracy calculation, hold reasons, and fail-closed adoption recommendation in `AnomalyClassificationEvaluationService.cs`. This split must not change thresholds, recommendation semantics, or presentation behavior. Covered by the isolated build, `--anomaly-classification-evaluation`, `--wpf-anomaly-purpose-flow`, `--wpf-labeling-shell`, and `--priority-workflow-docs`.
...
2026-07-27 anomaly classification dataset export ownership contract: keep the public output-root and normal/abnormal/skipped count result in `AnomalyClassificationDatasetExportContracts.cs`. Keep review-state loading, split selection, generated-folder reset, image copying, duplicate-name resolution, and count updates in `AnomalyClassificationDatasetExportService.cs`. This split must not change the classification folder layout or training workflow input. Covered by the isolated build, `--anomaly-classification-dataset-export`, `--anomaly-classification-training-workflow`, `--dataset-readiness-purpose`, and `--priority-workflow-docs`.
...
2026-07-27 YOLO segmentation training-label ownership contract: keep image, label-file, split polygon, background, empty-label, error, and readiness result state in `YoloSegmentationTrainingLabelContracts.cs`. Keep split traversal, OK background import and stale-copy cleanup, segment/mask loading, raster contour conversion, polygon normalization, label writing, and error collection in `YoloSegmentationTrainingLabelService.cs`. This split must not change generated labels, readiness semantics, or the read-only conversion used by remediation audit and migration. Covered by the isolated build, `--yolov8-segmentation-app-dataset-fixture`, `--dataset-readiness-purpose`, `--segmentation-historical-remediation-audit`, `--segmentation-template-contour-migration`, and `--priority-workflow-docs`.

2026-07-27 Yolo multi-public-file structural audit contract: outside `*Contracts.cs`, only `CYolov5.cs` and `YoloSegmentationAnnotationService.cs` intentionally contain more than one public type. Keep the 109-line legacy YOLOv5 YAML/training compatibility unit together until another adapter shares those contracts or its compatibility aliases can be retired. Keep the versioned segmentation annotation file/polygon/point schema with its storage/materialization service until an independent serializer or schema-version owner exists. Do not split either file for line count or public-type count alone, and do not restructure segmentation storage, mask materialization, Viewer/OpenGL/ROI/brush/eraser paths without a concrete requirement and focused evidence. The 2026-07-27 audit found 133 Yolo public declarations, zero duplicate public type names, 29 contract files owning 83 public types, and exactly these two non-contract multi-public files. Covered by the isolated build, `--wpf-settings-viewmodels`, `--segmentation-annotation-storage`, `--priority-workflow-docs`, declaration searches, and `git diff --check`.
...
2026-07-27 historical SEG remediation ownership contract: keep the public report/image/record/export result types in `YoloSegmentationHistoricalRemediationAuditContracts.cs`; keep legacy artifact discovery, read-only contour proposal, label diff, and Markdown writing in `YoloSegmentationHistoricalRemediationAuditService.cs`. This ownership split does not reopen the protected read-only behavior or authorize dataset changes. Covered by the isolated build, `--segmentation-historical-remediation-audit`, `--segmentation-template-contour-migration`, and `--priority-workflow-docs`.
...
2026-07-27 external evaluation data audit ownership contract: keep the public overlap/error report in `YoloExternalEvaluationDataAuditContracts.cs`; keep supported-image discovery, SHA-256 hashing, filename/content overlap calculation, and error collection in `YoloExternalEvaluationDataAuditService.cs`. The report's `IsIndependentByContent` remains only a byte-identical-copy preflight and not model-adoption evidence. Covered by the isolated build, `--external-evaluation-data-audit`, `--wpf-labeling-shell`, and `--priority-workflow-docs`.
...
2026-07-27 template-contour migration ownership contract: keep the public plan/item/result types in `YoloSegmentationTemplateContourMigrationContracts.cs`; keep plan construction, input hash verification, staging, backup, apply verification, and rollback in `YoloSegmentationTemplateContourMigrationService.cs`. The plan intentionally carries internal transaction state used between `BuildPlan` and `Apply`; moving it does not authorize migration without the existing operator-approved workflow. Covered by the isolated build, `--segmentation-template-contour-migration`, `--segmentation-historical-remediation-audit`, and `--priority-workflow-docs`.
...
2026-07-27 model-adapter catalog ownership contract: keep the public read-only adapter item in `ModelAdapterCatalogContracts.cs`; keep implemented-capability lookup and catalog composition in `ModelAdapterCatalogService.cs`. This split must not add adapters, unblock YOLO11, promote ONNX beyond inference-only use, or turn interchange support into runtime/model-quality evidence. Covered by the isolated build, `--model-adapter-catalog`, `--wpf-yolo-model-settings-panel`, `--wpf-labeling-shell`, and `--priority-workflow-docs`.
...
2026-07-27 segmentation mask comparison ownership contract: keep the public request/result, prediction run summary, per-class metrics, and prediction manifest record types in `SegmentationMaskComparisonContracts.cs`; keep compatibility validation, mask loading, Dice/IoU/component scoring, aggregation, and report writing in `SegmentationMaskComparisonService.cs`. The move must not change numerical behavior or relax canonical-export identity gates. Covered by the isolated build, `--segmentation-mask-comparison`, `--wpf-segmentation-adapter-comparison`, and `--priority-workflow-docs`.

2026-07-27 segmentation prediction export ownership contract: keep the public adapter request/result types in `SegmentationPredictionExportContracts.cs`; keep request construction/validation, Python process arguments, execution, and manifest-path resolution in `SegmentationPredictionExportService.cs`. This ownership split must not change adapter selection, runtime paths, output-directory safety, recipe/source immutability, or canonical artifact identity. Covered by the isolated build, `--segmentation-mask-comparison`, `--wpf-segmentation-adapter-comparison`, and `--priority-workflow-docs`; real runtime evidence remains the existing 2026-07-21 artifacts because no execution behavior changed.
...
2026-07-22 YOLO11 external segmentation and three-model comparison contract: the opt-in real external comparison runner defaults to YOLOv8 but may explicitly select YOLO11, and its summary must record the selected engine. The verified local `C:\Git\yolov8` Ultralytics runtime, bundled worker, and `yolo11n-seg.pt` seed completed a 30-epoch CPU `segment` run on an app-owned runtime copy of the fixed EasyMatch Die Array `360/80/60` packet. The original 2,004-file tree SHA-256 remained `5819E2ED72E402D3F06C32CF4F1FB3481A2DF1D70BD8CB8C00B97CE9E28199C2`; final checkpoint SHA-256 is `4A09B5F668B8F2AA2DAF9FEDB9ADDA4954A607D61CB08C96379AE8CA82462ECA`. The same 60 canonical test masks at YOLO confidence `0.25` measured mean Dice/IoU U-Net `0.243091/0.156165`, YOLOv8-seg `0.721702/0.570198`, and YOLO11-seg `0.773711/0.636553`, with no automatic adoption. Canonical artifact folders use a 24-character fingerprint prefix and prediction masks use collision-checked 32-character SHA prefixes, while manifests retain the full evidence hashes; a deliberately long real comparison path passed after this fix. Preserve the full-manifest identity checks and do not present this synthetic same-source ranking as production accuracy, comparable latency, or universal YOLO11 support. Evidence: `docs\SEGMENTATION_E30_THREE_MODEL_COMPARISON_20260722.md`.

2026-07-22 U-Net CE plus foreground soft-Dice rejection contract: one temporary opt-in, full-frame CUDA experiment kept the fixed 360 train / 80 valid packet, image 320, batch 4, seed 17, 30 epochs, unweighted cross-entropy, and the foreground-quality selector, while adding only a foreground-class soft-Dice term. The selected epoch-30 checkpoint SHA-256 is `B317D352661A9893C281A31B4C824154762E88673E958545C123B640AD960D33`; all 880 canonical train/valid image and mask hashes still matched their manifest. On the identical 80-image valid raster masks, `foreign_particle` recovered but `contamination_spot` stayed zero-overlap and macro Dice/IoU fell from `0.204437/0.127053` to `0.189220/0.111142`. The exact hypothesis is rejected, its temporary loss code is removed, the normal TCP worker is unchanged, no held-out test was used, and no checkpoint is registered or adopted. Do not repeat it unless data, architecture, loss definition, or acceptance criteria deliberately change. Evidence: `docs\UNET_E30_CLASS_CONFUSION_ANALYSIS_20260722.md` and `artifacts\unet-softdice-e30-valid-20260722\unet-valid-predictions`.

2026-07-22 MobileSAM box-prompt smart-mask contract: for a segmentation recipe only, the last non-empty operator rectangle may be sent to the bundled `openvisionlab_mobile_sam_box_prompt.py` worker using the existing local Ultralytics Python environment and an explicitly present `mobile_sam.pt`. The result is one `smart-mask` polygon candidate with no fabricated confidence score; it must remain pending until the normal candidate confirmation command, and only the existing label-save workflow may persist it. Confirmed candidates must be preserved, and image/prompt changes during inference must discard the stale result. No network call, automatic model download, automatic confirmation, or source-image write is allowed. A real synthetic defect prompt produced a 44-point polygon and non-empty 540-pixel mask; candidate confirmation wrote canonical segment JSON/mask PNG, while the source SHA-256 remained unchanged. Covered by Python compile/self-test, isolated build, `--mobile-sam-box-prompt`, `--real-mobile-sam-box-prompt`, candidate polygon save/shell/documentation gates, and current-build 1920x1080 prompt/result evidence under `artifacts\ui\smart-mask-20260722`. This proves labeling workflow behavior and provenance only; field validation is `Not evaluated`, no production accuracy is claimed, and point/negative/text/multi-object prompting remains outside the completed slice.

2026-07-22 MobileSAM eight-class exact-box usability contract: `Complete`. A fixed 24-image matrix selected one single-defect sample per class from each train/valid/test split of the synthetic hexagon packet. All 24 real worker calls succeeded and met IoU `>= 0.50`; overall median IoU was `0.8562`, and the lowest class median was `crack 0.7129`. The box-only gate passed, the 4,525-file source tree SHA-256 stayed `4E511A2E08F2ED609B78B40D6B789DE691C968E71ED5A298B76A1E7CA1FB52A8`, and runtime/weight provenance was recorded. Preserve the box-only plus manual-correction design. The later fixed box-jitter contract supersedes the earlier unevaluated small-box tolerance boundary; field accuracy remains unproven. Evidence: `docs\MOBILE_SAM_8_CLASS_USABILITY_MATRIX_20260722.md` and `artifacts\mobile-sam-usability-matrix\20260722-153003`.

2026-07-22 MobileSAM box-jitter robustness contract: `Complete` for the declared synthetic small-box-error range. The same fixed 24 images were each evaluated with 20% expansion, 10% contraction, and 10% translation in each diagonal direction. All 96 real worker calls met IoU `>= 0.50`; overall median IoU was `0.856132`, the lowest class median was `crack 0.704918`, and the lowest variant median was `shrink-10pct 0.850117`. The source remained 4,525 files with unchanged tree SHA-256 `4E511A2E08F2ED609B78B40D6B789DE691C968E71ED5A298B76A1E7CA1FB52A8`; runtime and weight identity matched the exact-box contract. Preserve box-only plus polygon/brush correction as regression behavior. The 2026-07-27 user-approved commercial-labeling roadmap supersedes the old “new reproducible failure required” priority rule and permits positive/negative prompts only as the separate acceptance-gated P0-B session in `docs\LABELING_EDITOR_COMMERCIAL_GAP_AND_ROADMAP_20260727.md`. Field validation and production accuracy remain `Not evaluated`. Evidence: `docs\MOBILE_SAM_BOX_JITTER_MATRIX_20260722.md` and `artifacts\mobile-sam-box-jitter-matrix\20260722-165800`.

2026-07-22 beginner task-workflow contract: `Complete`. The active documentation authority order is `AGENTS.md`, `docs/NEXT_THREAD_HANDOFF.md`, `docs/LABELING_STUDIO_COMPLETENESS_AUDIT.md`, then `docs/CODEX_NEXT_PROMPT.md`. Current object-detection box, segmentation brush/eraser, and anomaly close/restart/first-inference smokes passed with 1920x1080 evidence under `artifacts/ui/beginner-e2e-audit-20260722`. Canvas tool containers expose their bound automation names, and image-level anomaly candidates use whole-image OK/NG language instead of outside-image/overlap box language. Preserve object-detection candidate semantics and anomaly persistence. This is workflow evidence, not model accuracy or production adoption. Evidence: `docs/BEGINNER_END_TO_END_UX_AUDIT_20260722.md`.

2026-07-22 dataset-purpose generated-default contract: `Complete`. The dataset wizard synchronizes a new collision-checked generated Recipe name when purpose changes only if the initial name was actually generated and has never been operator-edited. Its default storage path follows only while that path is also untouched. An edit permanently protects the field for that wizard session, including edit-then-restore to the original text. A final current-EXE anomaly run proved `AnomalyDetection` in both generated fields, Recipe creation, YOLOv8 profile restore, first inference with one candidate, and persisted `Abnormal`. Preserve existing Recipe/data immutability and manual-path ownership. Evidence: `docs/DATASET_PURPOSE_AUTOMATIC_NAME_SYNC_20260722.md` and `artifacts/ui/dataset-purpose-auto-name-20260722/after`.

2026-07-22 image-queue action Worklist contract: `Complete`. The existing `Unlabeled` filter is exposed as one visible `확인 필요 Worklist` card and continues to derive membership from existing saved-label, anomaly-decision, candidate-review, and quality-review state. Saved labels, confirmed/skipped/no-candidate rows, and completed anomaly OK/NG decisions are excluded; unreviewed, save-required, candidate, failed, requested, and needs-fix rows remain. A single completion must use WPF live filtering and must not reset/rebuild the source or view. Filter counts without text search come from the same one-pass summary so status text cannot lag live filtering. Worklist-only normal label save explicitly advances from the completed image path; all other filter modes keep their existing save behavior. The 10,000-row gate retained all row instances, emitted no reset, evaluated the filter once, and updated the visible set from 5 to 4; final local synthetic timings were `4.3ms` summary and `113.3ms` completion/status update. In two consecutive actual-EXE runs, candidate, failed, needs-fix, and requested searches each returned `1/125` while a completed-label search returned `0/125`; save-required remained visible, then label save produced `completed=5->6`, `worklist=120->119`, zero queue invalidations/bulk changes, and active/selected `queue-local-001.jpg`. EXE SHA-256 is `B62AFCDF5B7820632CACF22C185DFC23E47E9F6844F7DAC30A79B5CBE531A70D`. Preserve the existing `다음 미완료` action. Do not add a second queue, DB, paging, collaboration, or per-object history without a newly reproduced gap. Evidence: `docs/IMAGE_QUEUE_ACTION_WORKLIST_20260722.md`, `artifacts/ui/image-queue-worklist-20260722`, and `artifacts/exe-image-queue-worklist/current-exe-20260722-categories-repeat`.

2026-07-27 P0-A labeling productivity contract: `Complete`. Purpose-filtered keys are `V` Select, `R` Rectangle, `P` Polygon, `B` Brush, `E` Eraser, and `H` PanZoom; `1~9` select the alphabetically deterministic visible class order and `0` opens Class Catalog for larger catalogs. `N` restores the last repeatable drawing tool plus class, and `Ctrl+D` duplicates only a selected manual box, polygon, or raster mask. Duplicate must deep-copy class/geometry/mask data, use a deterministic in-bounds 12px offset or same-position fallback when no movement is possible, register exactly one existing annotation-history snapshot, and continue through the canonical save/reopen path. TextBox, ComboBox, RangeBase, and TextBoxBase focus suppress these labeling shortcuts. The current tool/class remain selected after creation. The F1/help-button card stays in a separate Auto row above the WinForms/OpenGL canvas; do not place it back over the hosted canvas because actual-EXE airspace hides WPF overlays. Command mapping and duplicate geometry belong to `WpfAnnotationProductivityService`; visible repeat/class/help state belongs to `WpfCanvasPanelViewModel`; the shell remains the key/mutation bridge. Covered by the isolated build, `--labeling-productivity`, `--wpf-undo-redo-shortcuts`, ROI/segmentation/storage/mask/ROI performance gates, current-source before/after, and current-build `--exe-labeling-productivity-smoke`. Evidence: `docs/LABELING_EDITOR_COMMERCIAL_GAP_AND_ROADMAP_20260727.md` and `artifacts/ui/labeling-productivity-p0a-20260727`.

2026-07-27 P0-B interactive Smart Mask contract: `Complete`; field validation is `Not evaluated`. Segmentation keeps Rectangle available as the explicit start-box tool. `WpfSmartMaskPromptSessionService` owns image/Recipe/box/class identity, positive/negative points, point input mode, generation, 48/96/256 polygon detail, and next-instance eligibility. `WpfMobileSamBoxPromptService` and the bundled worker preserve box-only requests and add repeated labeled points plus a deterministic polygon-point limit. Rerun replaces one pending Smart Mask candidate while preserving confirmed labels. Image, Recipe, box, or point-generation mismatch discards stale results. Candidate creation never writes labels; only the existing confirm path writes canonical segment JSON/mask PNG. The prompt bar exposes point undo/clear, async worker cancellation, confirm/skip through Candidate Review, and next-instance box restoration. Protected overlays remain distinct: purple start box, green positive point, red negative point, blue pending candidate. Covered by Python compile/self-test; isolated and app Debug builds; `--mobile-sam-box-prompt`; `--real-mobile-sam-point-correction`; WPF labeling/purpose/segmentation/storage/performance regressions; `--priority-workflow-docs`; and current-build `--exe-smart-mask-point-smoke`. Real-worker evidence is `artifacts/mobile-sam-point-correction/20260727-185324`; actual-EXE evidence is `artifacts/ui/smart-mask-p0b-20260727`. Preserve the earlier 24-call exact-box and 96-call box-jitter contracts. Do not add automatic save/confirm, weight download, text prompt, task-wide segmentation, video tracking, or field-accuracy claims under this contract.

2026-07-27 P1-A segmentation interchange preservation/loss contract: `Complete`. `SegmentationInterchangeContractService` is the single owner for the canonical segment JSON, class-index mask PNG, YOLO segmentation, COCO polygon, and CVAT polygon capability matrix. Every target declares class identity, polygon geometry, raster pixels, holes, multiple components, instance grouping, z-order, and remove-underlying provenance as `Preserved`, `Conditional`, or `Lost`. COCO/CVAT/YOLO export result contracts expose deduplicated annotation-derived warnings; cutout fixtures must report `Holes: Lost` and must not be described as lossless conversion. At the P1-A checkpoint, segment JSON v1/v2 plus sibling mask PNG remained unchanged, and Viewer/OpenGL/brush/eraser paths were untouched. Covered by the isolated build, `--segmentation-interchange-contract`, existing segmentation storage/export/import regressions, `--priority-workflow-docs`, and `git diff --check`. Evidence: `docs/SEGMENTATION_INTERCHANGE_PRESERVATION_CONTRACT_20260727.md`. P1-B schema v3 is recorded in the following contract; structural editor commands remain P1-C.

2026-07-27 P1-B canonical segmentation schema v3 contract: `Complete`. New segment saves use schema version 3 and store persistent image-local object ID, component index, z-order, and last structural operation on every component record. One raster object with disconnected components emits one shared object ID and ordered component indices; load and re-save retain those values. Version 1/2 polygon and raster files remain readable with unchanged geometry and deterministic `legacy-NNNNNN` identity, component `0`, z-order `0`, and empty operation provenance. Annotation history clones preserve identity; `Ctrl+D` duplicates deliberately clear object/component identity and operation provenance before canonical save assigns a new identity. Mask stroke delta undo restores the same metadata. Covered by the isolated/app builds, `--segmentation-interchange-contract`, `--segmentation-annotation-storage`, `--labeling-productivity`, `--wpf-undo-redo-shortcuts`, COCO/CVAT import/export, YOLO segmentation fixture, WPF segmentation manipulation, and mask performance gates. Evidence: `docs/SEGMENTATION_INTERCHANGE_PRESERVATION_CONTRACT_20260727.md`. This is a storage contract, not merge/split/z-order editor parity.

2026-07-27 P1-C same-class segmentation merge/join contract: `Complete`. Saved Labels exposes a merge checkbox only for manual segmentation rows, a selected-count label, and the text/accessibility-backed `선택 마스크 병합` command. Two or more current same-class polygon or raster objects are unioned into one raster object; each polygon's cutouts are erased before source masks are OR-combined, so one source cutout cannot erase pixels supplied by another source. Mixed classes fail before mutation. The merged object gets a new object ID, component `-1`, maximum source z-order, and `LastStructuralOperation=Merge`; replacing all sources is exactly one existing full annotation-history step. Canonical v3 save/load/re-save retains the shared object identity, ordered disconnected component indices, and merge provenance. Geometry/validation belongs to `WpfSegmentationMergeService`; selection/count/enablement belongs to `WpfObjectReviewPanelViewModel`; the shell remains the mutation/history bridge. Viewer/OpenGL, ROI, brush, eraser, and mask-drag hot paths remain unchanged. Covered by the isolated/app builds, `--segmentation-merge`, `--segmentation-interchange-contract`, segmentation storage/manipulation, undo/redo, and mask-performance gates, current-source 1920x1080 before/after evidence, `--priority-workflow-docs`, and `git diff --check`. Evidence: `docs/SEGMENTATION_MERGE_P1C_20260727.md`. Manual hole editing, z-order UI, and remove-underlying remain separate P1-C work.

2026-07-27 P1-C axis-aligned segmentation split/slice contract: `Complete`. Saved Labels exposes text/accessibility-backed `세로 절단`, `가로 절단`, and cancel controls only for one selected manual segmentation object. The canvas bridge removes one pixel column or row and commits only when the remaining polygon/raster source forms at least two 4-connected components; edge/outside/non-separating clicks leave the source and history unchanged. `WpfSegmentationMaskGeometryService` is the shared merge/split polygon/raster conversion owner and applies polygon cutouts before structural editing. `WpfSegmentationSplitService` owns cut validation and deterministic component extraction. Every result is an independent raster object with a new ID, component `-1`, preserved class/z-order, and `LastStructuralOperation=Split`; replacement is one full annotation-history step. Pending input is cancelled by right-click, visible cancel, selected-object/image/tool change, and is rejected while Smart Mask owns point input. Canonical v3 save/load/re-save preserves generated IDs and provenance. Viewer/OpenGL, ROI, brush, eraser, and mask-drag hot paths remain unchanged. Covered by the isolated/app builds, `--segmentation-split`, merge/interchange/storage/history/segmentation/mask-performance regressions, current-source 1920x1080 before/after evidence, `--priority-workflow-docs`, and `git diff --check`. Evidence: `docs/SEGMENTATION_SPLIT_P1C_20260727.md` and `artifacts/ui/segmentation-split-p1c-20260727`. Manual hole editing, z-order, and remove-underlying remain separate P1-C work.

2026-07-28 P1-C segmentation hole editing contract: `Complete`. Saved Labels exposes text/accessibility-backed `구멍 그리기`, `구멍 채우기`, and cancel controls for one selected manual segmentation object. Hole-add accepts a polygon only when every rasterized pixel is selected-object foreground and the resulting empty component remains enclosed; hole-fill accepts only an empty component whose four-connected flood does not reach the object bounds. Boundary-crossing polygons, foreground clicks, concavities, and exterior-connected open channels are rejected on copied geometry before mutation. `WpfSegmentationHoleService` owns validation and flood fill; `WpfObjectReviewPanelViewModel` owns enablement/guidance; `WpfLabelingShellWindow.SegmentationHoleCommands` owns point input, mutation, history, and selection. Polygon/raster output keeps object ID/class/z-order, uses component `-1`, and records `HoleAdd` or `HoleRemove`; each successful operation is one full annotation-history step. Pending input is cancelled by right-click, visible cancel, selected-object/image/tool change, and cannot overlap Smart Mask point input. Canonical v3 save/load/re-save preserves identity and provenance. Viewer/OpenGL, ROI, brush, eraser, and mask-drag hot paths remain unchanged. Covered by isolated/app builds, `--segmentation-hole`, protected structural/storage/history/segmentation/mask-performance regressions, current-source 1920x1080 evidence, `--priority-workflow-docs`, and `git diff --check`. Evidence: `docs/SEGMENTATION_HOLE_P1C_20260728.md` and `artifacts/ui/segmentation-hole-p1c-20260728`. Z-order and remove-underlying remain separate P1-C work.

2026-07-28 P1-C saved-object z-order contract: `Complete`. Saved Labels exposes text/accessibility-backed `맨 뒤`, `한 칸 뒤`, `한 칸 앞`, and `맨 앞` controls for one selected manual segmentation object. `WpfSegmentationZOrderService` stable-sorts by existing z-order/current position, rejects first/last boundary moves before mutation, and plans one deterministic move. The shell normalizes the resulting global stack to `0..N-1`, preserves object ID/class/polygon/cutout/raster/component data and selection, records `LastStructuralOperation=ZOrder` on affected objects, and creates exactly one full annotation-history step. Canonical v3 save/load/re-save and shell reload preserve global order across class-grouped storage. `AnnotationPolygonOverlays` respects this order within each polygon or raster overlay family; exact polygon/raster interleaving is not claimed because the Viewer still uses separate render passes. Viewer/OpenGL, ROI, brush, eraser, and mask-drag hot paths remain unchanged. Covered by isolated/app builds, `--segmentation-zorder`, protected structural/storage/history/segmentation/mask-performance regressions, current-source 1920x1080 evidence, `--priority-workflow-docs`, and `git diff --check`. Evidence: `docs/SEGMENTATION_ZORDER_P1C_20260728.md` and `artifacts/ui/segmentation-zorder-p1c-20260728`. Remove-underlying remains the next P1-C slice and requires affected-object preview/warning and explicit confirmation.

2026-07-29 Object Review persistent metadata contract: `Complete`. Object Review is the named consumer for saved manual box/segmentation `occluded` state and Recipe-defined tags. Keep Recipe tag definitions visible, editable, normalized, resettable, and restored without executing annotation work. Keep per-image data in sidecars under `data/<split>/object-metadata/<stem>.json`; schema v2 writes group IDs and schema v1 remains readable. Do not embed metadata in YOLO labels. Boxes reconnect by class/bounds/duplicate occurrence and segments by stable object ID. Metadata changes mark annotations dirty and write only after explicit label save; default state deletes a stale sidecar. Preserve combined `가림` plus tag filters, row badges, and session-only separation for hide/full-lock/movement-pin. Training weighting, external interchange, candidate metadata, and automatic save remain outside this contract. Covered by the isolated build, `--object-metadata-review`, Object Review/session/shell/annotation/project-config regressions, current-source 1920x1080 and 1366x768 captures, `--priority-workflow-docs`, and `git diff --check`. Evidence: `docs/OBJECT_METADATA_REVIEW_CONSUMER_P4_20260729.md` and `artifacts/ui/object-metadata-review-20260729`.

2026-07-29 same-image Object Review grouping: `Complete`. Two or more saved manual boxes, polygons, or raster masks can form one image-local physical/review unit. The dedicated group-selection set is separate from segmentation merge, previews member count/types/classes, and supports explicit create/cancel. Object Review shows image-local group badges, grouped/ungrouped/specific-group filters, member removal, confirmed dissolve, and group-level `가림`/Recipe-tag actions. Schema-v2 sidecars remain v1-readable and write only on explicit label save. Delete dissolves singletons, duplicate stays ungrouped, merge inherits only one shared nonempty group, and split outputs inherit the source group. Geometry merge/shared movement, training, interchange, cross-image/video, collaboration, and automatic save remain excluded. Covered by isolated build, `--object-group-review`, metadata/Object Review/session/merge/split/shell/history regressions, three consecutive 100,000-row settings-viewmodel passes, default 260/260 suite, responsive checks, and current-source before/after captures. Evidence: `docs/OBJECT_GROUP_REVIEW_IMPLEMENTATION_P5_20260729.md` and `artifacts/ui/object-group-review-20260729`.

2026-07-28 P1-C remove-underlying contract: `Complete`; field validation is `Not evaluated`. Saved Labels exposes accessible `겹침 분석`, `확인 후 제거`, and `취소`. `WpfSegmentationRemoveUnderlyingService` stable-orders by `(ZOrder, current index)`, rasterizes polygon/cutout/raster objects through the shared geometry service, and computes a read-only plan containing affected objects, removed pixels, full removals, exact raster remainders, and a full geometry/order signature. Preview and cancel do not mutate or create history; affected overlays and the pending summary are orange. Apply re-analyzes and rejects stale plans. Only lower overlapping objects change: partial survivors preserve ID/class/z-order with component `-1` and `LastStructuralOperation=RemoveUnderlying`, full coverage removes the object, and selected/front/unaffected objects remain. One Apply is one full Undo/Redo step. Canonical v3 save/load/re-save preserves the raster remainder, identity, z-order, and provenance. Viewer/OpenGL, ROI, brush, eraser, and mask-drag hot paths remain unchanged. Covered by the isolated build, `--segmentation-remove-underlying`, protected structural/storage/history/segmentation/mask-performance regressions, and current-source 1920x1080 before/after evidence. Evidence: `docs/SEGMENTATION_REMOVE_UNDERLYING_P1C_20260728.md` and `artifacts/ui/segmentation-remove-underlying-p1c-20260728`. P1-C is complete; its historical next dependency was P2 session-only hide/lock/pin, completed in the following record.

2026-07-28 P2 session-only object-state contextual correction: `Complete`; field validation is `Not evaluated`. The first record's gold-review-bookmark interpretation of pin was invalidated by re-checking the supplied CVAT video. The corrected contract is hide, full lock, and movement pin: pin blocks whole-object translation while preserving ROI resize, polygon vertex edit, copy, delete, class, and structural commands; ordinary class colors remain and `PINNED` overlay labels are prohibited. Object-state icons are compact, quality review and segmentation structural commands are collapsed options, and the segment editor is visible only for a selected manual segment. Zero-warning/error isolated/application builds, `--object-session-state`, protected structural/storage/history/ROI/segmentation/mask-performance/Object Review/shell/MVVM/productivity/MobileSAM regressions, and fresh current-build 1920x1080/1366x768 comparison pass. The 1366 baseline had zero visible object rows; the after capture shows all three fixture rows. Evidence: `docs/OBJECT_SESSION_STATE_P2_20260728.md`, `docs/OBJECT_REVIEW_CONTEXTUAL_UI_CORRECTION_20260728.md`, and `artifacts/ui/object-review-contextual-20260728`. Labeling-editor depth is `3.1/5`; focused workstation maturity remains `4.0/5`. P2 polygon vertex insert/delete is next. Recommended model: `gpt-5.6-sol`; reasoning effort: `high`.

2026-07-28 P2 polygon vertex insert/delete: `Complete`; field validation is `Not evaluated`. Selected saved manual polygons expose insert/delete only inside the collapsed segment context. Eight screen pixels are converted through current zoom; insert projects onto the deterministic nearest edge and delete selects the nearest vertex. Endpoint, duplicate, triangle, zero-area, and self-intersecting outcomes reject without mutation. Successful edits preserve object ID/class/component/z-order/cutouts/selection, create one Undo/Redo step, and replay `VertexInsert`/`VertexDelete` through canonical v3. Hidden/full-lock paths reject direct commands; movement pin allows vertex editing. Zero-warning/error isolated build, `--polygon-vertex`, protected structure/state/interchange/history/Object Review/ROI/segmentation gates, and fresh current-build 1920x1080/1366x768 captures pass. Evidence: `docs/POLYGON_VERTEX_EDIT_P2_20260728.md` and `artifacts/ui/polygon-vertex-p2-20260728`. At this checkpoint labeling-editor depth was `3.2/5`; focused workstation maturity remained `4.0/5`. Its historical next dependency was edge-aware intelligent scissors, completed in the following record. Recommended model: `gpt-5.6-sol`; reasoning effort: `high`.
...
2026-07-28 Smart Mask auto-first contextual correction contract: `Complete` in the current uncommitted worktree; field validation is `Not evaluated`. A new session leads with the automatic filled candidate and one `보정 옵션` disclosure. Positive/negative points, point undo/clear, generation cancel, and 48/96/256 boundary detail stay hidden until explicitly requested. Expansion persists for reruns of the same object and collapses on session end/image/next-object reset. Candidate Review, explicit confirm/skip, no pending autosave, MobileSAM worker behavior, and canonical save remain unchanged. Covered by the zero-warning/error isolated build, `--mobile-sam-box-prompt`, `--smart-mask-auto-boundary-presentation`, `--priority-workflow-docs`, actual Debug EXE contextual-correction replay, and current-source 1920x1080 compact/expanded evidence. Evidence: `docs/SMART_MASK_CONTEXTUAL_CORRECTION_UX_20260728.md`, `artifacts/ui/smart-mask-contextual-correction-20260728`, and `artifacts/operator-video/20260728-smartmask-contextual-correction`.

2026-07-28 Smart Mask real correction-effectiveness contract: `Complete`; field validation is `Not evaluated`. A fixed two-development/four-held-out KolektorSDD replay preserves each poor baseline, then evaluates positive-only, negative-only, and combined prompts. Positive-only reduces false negatives in `6/6`; negative-only reduces false positives in `4/4` applicable cases; combined correction improves held-out `3/4` with median IoU delta `+0.0988`. Two worsening combined trials remain in the evidence, including `kos14_Part7` baseline `0.3749`, positive-only `0.4620`, combined `0.3260`. Product guidance is one point -> rerun/compare -> next point only if needed. The 798-file source tree hash remains unchanged. Evidence: `docs/SMART_MASK_CORRECTION_EFFECTIVENESS_20260728.md` and `artifacts/mobile-sam-correction-effectiveness/20260728-202434`. Ground-truth-guided point choice is an evaluation oracle and does not prove unaided operator clicks or production accuracy.
...
## Operational viewer stability during Smart Mask review (2026-08-05)

Status: stable for the current-workstation labeling workflow.

- Inference result application must not force immediate, Render, or
  ApplicationIdle `ZoomToFit` calls. `MainCanvasView.SizeChanged` owns one
  coalesced ContextIdle fit only after a real viewport-size change.
- Detection summaries and Smart Mask session guidance must remain non-sizing
  surfaces. Their visibility must not consume or release viewer height.
- Dynamic annotation actions must retain the shared 29px toolbar line height
  when wrapping.
- Candidate decisions remain explicit in Candidate Review; result visibility
  never confirms, skips, saves, or changes the active layer automatically.
- The actual EXE preserved image bounds `877,383,232,589` across six workflow
  checkpoints, and all 65 public-GIF frames preserved scaled bounds
  `468,204,123,314`.

Covered by zero-warning/error isolated/current builds, the focused canvas,
detection-display, and batch-detection tests, actual-EXE auto-contour
confirm/save smoke on the leftmost monitor, frame-bound analysis, public media
review, documentation gates, and the solo `267/267` protected suite.
