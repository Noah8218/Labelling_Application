...
1. 박스 기반 MobileSAM 스마트 마스크는 8개 합성 결함 클래스의 정확한 박스 24회와 작은 확대·축소·이동 96회 평가를 완료했습니다. 점·음성 프롬프트는 실제 사용에서 박스만으로 부족한 사례가 재현될 때만 확장합니다.
...
- MobileSAM은 현재 단일 박스 프롬프트와 사람의 후보 확정만 지원합니다. 고정 평가에서는 정확한 메타데이터 박스와 20% 확대, 10% 축소, 10% 대각선 이동까지 검증했지만 더 큰 오차, 임의 형상, 다중 객체와 현장 정확도는 보장하지 않습니다. 자동 확정, 점/음성 프롬프트와 텍스트 기반 자동 라벨링은 지원하지 않습니다.
