...
{"imgSize":"640","batch":"8","epoch":"100","cfg":"yolov5m.yaml","weight":"yolov5m.pt","dataYaml":"<dataset-root>/data.yaml"}
```
...
Set the Python-side project directory in `ProjectSettings.PythonModel`. The C# app can auto-start the client when training or inference is requested.

Manual startup is still useful for Python-side debugging. Start the C# labeling app first, then run from that folder:

```powershell
py .\labelling_tcp_client.py --retry --weights "<model-root>\best.pt" --image-root "<dataset-root>"
```
