...
{
    public partial class AnnotationViewer : Component
    {
        ...
        private AnnotationRectangleObject _TempOb = new AnnotationRectangleObject();
        ...
        private Dictionary<string, List<AnnotationRectangleObject>> _RoiDic = new Dictionary<string, List<AnnotationRectangleObject>>();
        ...
        internal IReadOnlyDictionary<string, List<AnnotationRectangleObject>> RoiByClass => _RoiDic;
        ...
        public AnnotationViewer(bool bCenter = true)
        {
        ...
        public void SetSelectedClass(Yolo.LabelClass classItem)
        {
            ...
            _TempOb.cClassItem = classItem ?? new Yolo.LabelClass { Text = _SelectedClass };
        ...
        public void SetRoiRectangles(IEnumerable<Rectangle> rectangles, Yolo.LabelClass classItem = null, bool reset = true)
        {
            ...
            Yolo.LabelClass targetClass = ResolveRoiClass(classItem);
            string className = targetClass.Text;

            if (!_RoiDic.TryGetValue(className, out List<AnnotationRectangleObject> rois))
            {
        ...
        public List<Rectangle> GetRoiRectangles()
        {
            ...
            foreach (KeyValuePair<string, List<AnnotationRectangleObject>> roiGroup in _RoiDic)
            {
            ...
            bool reset = true)
        {
            ...
            {
                Yolo.LabelClass classItem = ResolveClassItem(group.Key, classes);
        ...
        public void SetSegmentationPolygons(IEnumerable<IEnumerable<Point>> polygons, Yolo.LabelClass classItem = null, bool reset = true)
        {
            if (reset)
            {
                _SegmentationDic.Clear();
        ...
        public bool AddSegmentationPolygon(IEnumerable<Point> polygon, Yolo.LabelClass classItem = null, bool refresh = true, bool select = true, bool recordUndo = true)
        {
            ...
            Yolo.LabelClass targetClass = ResolveSegmentationClass(classItem);
            string className = targetClass.Text;
        ...
        public int AddSegmentationBrushStamps(IEnumerable<Point> centers, int radius = DefaultSegmentationBrushRadius, Yolo.LabelClass classItem = null)
        {
            if (_currentImage == null)
            {
                return 0;
        ...
        public int AddAutoSegmentationFromRois(Yolo.LabelClass classItem = null, bool onlySelected = true)
        {
            ...
            List<AnnotationRectangleObject> rois = _RoiDic
                .SelectMany(group => group.Value ?? new List<AnnotationRectangleObject>())
            ...
            AnnotationSnapshot before = CaptureAnnotationSnapshot();
            int added = 0;
            foreach (AnnotationRectangleObject roi in rois)
            {
        ...
        public int MergeSegmentationSegments(string className = null)
        {
            ...
            AnnotationSnapshot before = CaptureAnnotationSnapshot();
            Yolo.LabelClass targetClass = mergeTargets.FirstOrDefault(segment => segment?.ClassItem != null)?.ClassItem
                ?? ResolveSegmentationClass(new Yolo.LabelClass { Text = targetClassName });
        ...
        public List<LabelingRoiListItem> GetRoiListItems()
        {
            ...
            foreach (KeyValuePair<string, List<AnnotationRectangleObject>> roiGroup in _RoiDic)
            {
                for (int i = 0; i < roiGroup.Value.Count; i++)
                {
                    AnnotationRectangleObject roi = roiGroup.Value[i];
        ...
        private void OnCanvasMouseMove(object sender, CanvasMouseEventArgs e)
        {
            ...
            if (!isPointerDown)
            {
                // Legacy AnnotationViewer hover updates status/cursor only. Repainting here makes
        ...
        private void SelectRoiAt(Point imagePoint)
        {
            ...
            foreach (KeyValuePair<string, List<AnnotationRectangleObject>> roiGroup in _RoiDic.Reverse())
            {
                List<AnnotationRectangleObject> rois = roiGroup.Value;
        ...
        private void UpdateRectangleDrawing(MouseEventArgs e, int distanceX, int distanceY)
        {
            ...
            if (!_RoiDic.TryGetValue(_SelectedClass, out List<AnnotationRectangleObject> rois))
            {
                return;
            }
        ...
        private bool PaintSegmentationBrushStroke(IReadOnlyCollection<Point> centers)
        {
            ...
            Yolo.LabelClass targetClass = ResolveSegmentationClass(_TempOb.cClassItem);
            string className = targetClass.Text;
        ...
        private int AddBrushStroke(IEnumerable<Point> centers, int radius, Yolo.LabelClass classItem)
        {
            if (_currentImage == null)
            {
                return 0;
        ...
        private int RemoveSegmentsByStroke(IEnumerable<Point> centers, int radius)
        {
            ...
            foreach (string className in _SegmentationDic.Keys.ToList())
            {
                ...
                Yolo.LabelClass classItem = segments.FirstOrDefault(segment => segment?.ClassItem != null)?.ClassItem
                    ?? new Yolo.LabelClass { Text = className, DrawColor = Color.LimeGreen };
        ...
        private LabelingSegmentationObject GetOrCreateRasterSegment(string className, Yolo.LabelClass classItem)
        {
            if (!_SegmentationDic.TryGetValue(className, out List<LabelingSegmentationObject> segments))
            {
                segments = new List<LabelingSegmentationObject>();
        ...
        {
            Yolo.LabelClass targetClass = ResolveRoiClass(_TempOb.cClassItem);
            UnSelectAll();
            AnnotationRectangleObject rectangleObject = new AnnotationRectangleObject
            {
                Roi = TempROI,
                cClassItem = targetClass,
                Selected = true
            ...
            string className = targetClass.Text;
            if (!_RoiDic.TryGetValue(className, out List<AnnotationRectangleObject> list))
            {
        ...
        private bool RemoveSelectedRoi()
        {
            ...
            {
                List<AnnotationRectangleObject> rois = _RoiDic[className];
            ...
            if (!string.IsNullOrWhiteSpace(_SelectedClass)
                && _RoiDic.TryGetValue(_SelectedClass, out List<AnnotationRectangleObject> selectedClassRois)
                && _SelectROiIndex >= 0
        ...
        private int FindSelectedAnnotationListIndex()
        {
            ...
            foreach (KeyValuePair<string, List<AnnotationRectangleObject>> roiGroup in _RoiDic)
            {
                foreach (AnnotationRectangleObject roi in roiGroup.Value ?? new List<AnnotationRectangleObject>())
                {
                    if (roi == null || roi.Roi.IsEmpty)
                    {
                        continue;
        ...
        {
            ClearRasterMaskTextureCache();
            _RoiDic = new Dictionary<string, List<AnnotationRectangleObject>>();
            ...
            foreach (KeyValuePair<string, List<RoiSnapshot>> group in snapshot?.Rois ?? new Dictionary<string, List<RoiSnapshot>>())
            {
                ...
                {
                    var classItem = new Yolo.LabelClass
                    {
        ...
        private void UpdateCanvasCursor()
        {
            ...
            AnnotationRectangleObject selected = GetSelectedRoiObject();
            if (selected == null)
            {
                Canvas.Cursor = Cursors.Default;
                return;
        ...
        private AnnotationRectangleObject GetSelectedRoiObject()
        {
            if (string.IsNullOrEmpty(_SelectedClass)
                || !_RoiDic.TryGetValue(_SelectedClass, out List<AnnotationRectangleObject> selectedList)
                || selectedList == null
                || _SelectROiIndex < 0
            ...
            AnnotationRectangleObject selected = selectedList[_SelectROiIndex];
        ...
        {
            LabelingApplicationState.Inst.LabelingWorkflow.CommitMainAnnotations(LabelingApplicationState.Inst.Data, LabelingApplicationState.Inst.System);
        }
        ...
        private Yolo.LabelClass ResolveSegmentationClass(Yolo.LabelClass classItem)
        {
            Yolo.LabelClass targetClass = classItem ?? _TempOb.cClassItem ?? new Yolo.LabelClass { Text = _SelectedClass };
        ...
        private Yolo.LabelClass ResolveRoiClass(Yolo.LabelClass classItem)
        {
            Yolo.LabelClass targetClass = classItem ?? _TempOb.cClassItem ?? new Yolo.LabelClass { Text = _SelectedClass };
            string className = targetClass.Text ?? _SelectedClass ?? string.Empty;
            if (string.IsNullOrWhiteSpace(className))
        ...
        {
            Color classColor = LabelingApplicationState.Inst?.Data?.ClassNamedList?
        ...
        private static Yolo.LabelClass ResolveClassItem(string className, IReadOnlyList<Yolo.LabelClass> classes)
        {
            Yolo.LabelClass classItem = classes?.FirstOrDefault(item => string.Equals(item?.Text, className, StringComparison.OrdinalIgnoreCase))
                ?? new Yolo.LabelClass { Text = className, DrawColor = Color.LimeGreen };
